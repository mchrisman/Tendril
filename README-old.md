...
# Tendril: Pattern Matching for JSON-like Structures

**Tendril** is a single, coherent language for matching, joining, and transforming JSON-like structures.
It combines:

* structural **pattern matching** (in the spirit of regex),
* **path-based** access (à la jq or JSONPath),
* **relational joins** across data,
* and logic-style **variable binding**,
  in one declarative matching model.
  As a result, Tendril:
* works directly on **messy, real JSON-like data**,
* can **validate and extract** in a single expression,
* and scales from **trivial filters** to **global, cross-field constraints** without changing tools.
...
.mutate({
  ...
  p: b => ({placeholder: b.text})  // move its text into the <input>
});
```
...
.on({users, orders})
.solutions()
// → [{i:0, j:0, userId:1, name:"Alice", item:"laptop"},
//    {i:1, j:1, userId:2, name:"Bob", item:"mouse"},
//    {i:1, j:1, userId:2, name:"Bob", item:"mousepad"}]
```
...
.solve()
// → {
//     item: "broccoli",  // last key iterated
...
```
Tendril("[... (2|4 as $x) (_ as $y) ...]").on([1, 2, 3, 4, 5]).solutions()  // [{x:2,y:3}, {x:4,y:5}]
```
...
```
Tendril("[$x $x]").in([1, [2, 2]]).mutate({x: _ => ['the','replacement']}) // ['the','replacement'] treated as a scalar
   // -> [1, ['the','replacement'], ['the','replacement']]

Tendril("[@x @x]").in([1, [2, 2]]).mutate({x: _ => ['the','replacement']})  // ['the','replacement'] treated as a group/slice
   // -> [1, ['the','replacement', 'the','replacement']]
```
...
```
Tendril("{ (/a/:_, /b/:_ as %x) /c/:_ }").on({Big:1, Cute:2, Alice:3}).solve() // {x:{Big:1, Alice:3}}
Tendril("{ (/a/:_, /b/:_ as %x) /c/:_ }").on({Big:1, Cute:2, Alice:3}).mutate({x:_=>{foo:"bar"}}) // -> {foo:"bar",Cute:2}
```
...
Tendril("{ a.b.c[3].e:f }").on(object).test()
...
// Simple API (90% of use cases) — returns plain objects
pattern.on(data).test()        // boolean: does pattern match?
pattern.on(data).solve()       // first solution as plain object, or null
pattern.on(data).solutions() // array of all solutions as plain objects

pattern.in(data).count()       // number of matches anywhere in data
pattern.in(data).locations()   // array of {path, fragment, bindings}

// Advanced API — returns rich OccurrenceSet/SolutionSet objects
let occSet = pattern.advancedMatch(data)  // anchored match at root
let occSet = pattern.advancedFind(data)   // find at any depth (recursive scan)
let occSet = pattern.first(data)          // first occurrence only (short-circuits)

// Legacy aliases still work:
pattern.match(data)  // alias for advancedMatch
pattern.find(data)   // alias for advancedFind
...
```javascript
// Simple API — replace() and mutate()
const result = pattern.on(data).replace(99);             // replace entire match
const result = pattern.on(data).replace(b => b.x * 2);   // with function
const result = pattern.on(data).mutate({x: 99});         // replace named variables
const result = pattern.on(data).mutate(b => ({x: b.y, y: b.x})); // with function

// Search and replace all occurrences
const result = pattern.in(data).replace(99);             // replace all matches
const result = pattern.in(data).mutate({x: 99});         // mutate all matches

// Advanced API — replaceAll() and editAll() on OccurrenceSet
const result = pattern.advancedFind(data).replaceAll(99);
const result = pattern.advancedFind(data).replaceAll($ => $.x * 2);
const result = pattern.advancedFind(data).editAll({x: 99});
const result = pattern.advancedFind(data).editAll($ => ({x: $.y, y: $.x}));

// Per-occurrence or per-solution editing (advanced)
...
pattern.advancedFind(data).editAll({x: 99}, {mutate: true});
```
...
Tendril("[$x ...]").on([[1,2], 3]).mutate({x: [9,9]})
...
.in(data)
.mutate({slice: {sanitized: true}});  // Adds sanitized:true everywhere.
...
```javascript
// Swap array elements
Tendril("[$x $y]").on([3, 4]).replace(b => [b.y, b.x])
// → [4, 3]

// Redact passwords at any depth - two equivalent approaches:
const data1 = {user: {password: "secret", name: "Alice"}};

// Approach 1: Use ** with .on() to search at any depth from root
const result1 = Tendril("{ **.password:$p }").on(data1).mutate({p: "REDACTED"});

// Approach 2: Use .in() without ** to search at any depth
const result2 = Tendril("{ password:$p }").in(data1).mutate({p: "REDACTED"});

// ⚠️ Anti-pattern: Don't combine ** with .in() - it's redundant!
// Tendril("{ **.password:$p }").in(data) // DON'T DO THIS
```
...
Tendril("%{ foo:1 bar:2 }").in(data).replace({replaced: true})
// → {replaced: true, baz: 3}

// Array slice: replace just the matched subsequence
const arr = [1, 2, 3, 4, 5];
Tendril("@[ 2 3 ]").in(arr).replace([20, 30, 31])
// → [1, 20, 30, 31, 4, 5]
...
**Important:** Slice patterns only work with `.in()` (search) and `first()`. Using them with `.on()` (anchored match) is an error—there's no surrounding container at the root level.
...
Tendril(`{
  ...
  **:{
  ...
  placeholder: b => ({placeholder: b.labelText})
})
```
...
Do you want a simple **find and extract**?

```
data = {
    users = {
        1001: {name:"Alice", active:false},
        1002: {name:"Bob", active:true},
        1003: {name:"Charlie", active:false},
        1004: {name:"Eunice", active:true},
        1005: {name:"Dawn", active:true}
    }
}
Tendril(`{
    users: {_: {name:$name, active:true}},
}`)
.on(data).solutions()
```

Do you want **patterns that look like the data?**

```
// Extract settled, non-cancelled, low-risk payments
record = {
  "order": {"id":"o_7712", "status":"pending", "items":2},
  "actions": [
   {"type":"capture","code":"soft_decline",
    "attempt":{"id":"pa_01","amt":1299,"net":"card"}},
   {"type":"capture","code":"approved",
    "attempt":{"id":"pa_02","amt":1299,"net":"card"}},
   {"type":"debug-auth", "id":"0843778"},
   {"type":"settle", "amt":1299}
  ],
  "audit": {"risk.scalp":4, "risk.fraud":7}
}
Tendril(`{
    order: {id: $orderId}, 
    actions: [(! _* {type:cancel} ) 
              _* {type:capture, code:approved, 
                  attempt:{id:$captureId, amt:$amount}}
              _* {type:settle, amt:$amount} 
              _* ]
    audit: { each /risk.*/:(_ where _<10)  }
}`)
.on(record).solve()
// => {orderId:"o_7712", amount:1299}
```

or **like paths** (same result, and grab indices too)?

```
Tendril(`{
    order.id: $orderId
    (!actions[_].type:cancel)
    actions[$i].type: capture
    actions[$i].code: approved
