 ...
 *
 * Tests for guard expressions: $x=(PATTERN where EXPR)
 *
...
test('guard: $x > 100 matches numbers greater than 100', () => {
  const result = Tendril('$x=(_number where $x > 100)').match(150).solutions().first();
  assert.ok(result);
  assert.equal(result.x, 150);
});

test('guard: $x > 100 does not match numbers <= 100', () => {
  assert.ok(!Tendril('$x=(_number where $x > 100)').match(50).hasMatch());
  assert.ok(!Tendril('$x=(_number where $x > 100)').match(100).hasMatch());
});

test('guard: $x < 0 matches negative numbers', () => {
  const result = Tendril('$x=(_number where $x < 0)').match(-5).solutions().first();
  assert.ok(result);
  assert.equal(result.x, -5);
});

test('guard: equality comparison', () => {
  assert.ok(Tendril('$x=(_number where $x == 42)').match(42).hasMatch());
  assert.ok(!Tendril('$x=(_number where $x == 42)').match(41).hasMatch());
});

test('guard: inequality comparison', () => {
  assert.ok(Tendril('$x=(_number where $x != 0)').match(1).hasMatch());
  assert.ok(!Tendril('$x=(_number where $x != 0)').match(0).hasMatch());
});

test('guard: less than or equal', () => {
  assert.ok(Tendril('$x=(_number where $x <= 10)').match(10).hasMatch());
  assert.ok(Tendril('$x=(_number where $x <= 10)').match(5).hasMatch());
  assert.ok(!Tendril('$x=(_number where $x <= 10)').match(11).hasMatch());
});

test('guard: greater than or equal', () => {
  assert.ok(Tendril('$x=(_number where $x >= 10)').match(10).hasMatch());
  assert.ok(Tendril('$x=(_number where $x >= 10)').match(15).hasMatch());
  assert.ok(!Tendril('$x=(_number where $x >= 10)').match(9).hasMatch());
});
...
test('guard: && (and) operator', () => {
  // x must be > 0 AND < 10
  assert.ok(Tendril('$x=(_number where $x > 0 && $x < 10)').match(5).hasMatch());
  assert.ok(!Tendril('$x=(_number where $x > 0 && $x < 10)').match(0).hasMatch());
  assert.ok(!Tendril('$x=(_number where $x > 0 && $x < 10)').match(15).hasMatch());
});

test('guard: || (or) operator', () => {
  // x must be < 0 OR > 100
  assert.ok(Tendril('$x=(_number where $x < 0 || $x > 100)').match(-5).hasMatch());
  assert.ok(Tendril('$x=(_number where $x < 0 || $x > 100)').match(150).hasMatch());
  assert.ok(!Tendril('$x=(_number where $x < 0 || $x > 100)').match(50).hasMatch());
});
...
test('guard: short-circuit && (false && X does not evaluate X)', () => {
  ...
  assert.ok(!Tendril('$x=(_number where false && $x > 0)').match(5).hasMatch());
});

test('guard: short-circuit || (true || X does not evaluate X)', () => {
  // This tests that true || error-inducing-expr doesn't throw
  assert.ok(Tendril('$x=(_number where true || $x / 0 > 0)').match(5).hasMatch());
});

// ==================== Arithmetic Operators ====================

test('guard: addition', () => {
  assert.ok(Tendril('$x=(_number where $x + 10 == 15)').match(5).hasMatch());
});

test('guard: subtraction', () => {
  assert.ok(Tendril('$x=(_number where $x - 5 == 10)').match(15).hasMatch());
});

test('guard: multiplication', () => {
  assert.ok(Tendril('$x=(_number where $x * 2 == 10)').match(5).hasMatch());
});

test('guard: division', () => {
  assert.ok(Tendril('$x=(_number where $x / 2 == 5)').match(10).hasMatch());
});

test('guard: modulo', () => {
  // x must be even
  assert.ok(Tendril('$x=(_number where $x % 2 == 0)').match(4).hasMatch());
  assert.ok(!Tendril('$x=(_number where $x % 2 == 0)').match(3).hasMatch());
});

test('guard: division by zero fails match', () => {
  assert.ok(!Tendril('$x=(_number where $x / 0 > 0)').match(5).hasMatch());
});
...
test('guard: size() on string', () => {
  const result = Tendril('$x=(_string where size($x) > 5)').match('hello world').solutions().first();
  assert.ok(result);
  assert.equal(result.x, 'hello world');
});

test('guard: size() on string - fails', () => {
  assert.ok(!Tendril('$x=(_string where size($x) > 10)').match('hi').hasMatch());
});

test('guard: size() on array', () => {
  const result = Tendril('$x=(_ where size($x) == 3)').match([1, 2, 3]).solutions().first();
  assert.ok(result);
});

test('guard: size() on object', () => {
  const result = Tendril('$x=(_ where size($x) == 2)').match({a: 1, b: 2}).solutions().first();
  assert.ok(result);
});

test('guard: number() conversion', () => {
  assert.ok(Tendril('$x=(_string where number($x) > 100)').match('150').hasMatch());
  assert.ok(!Tendril('$x=(_string where number($x) > 100)').match('50').hasMatch());
});

test('guard: string() conversion', () => {
  assert.ok(Tendril('$x=(_number where string($x) == "42")').match(42).hasMatch());
});

test('guard: boolean() conversion', () => {
  assert.ok(Tendril('$x=(_number where boolean($x))').match(1).hasMatch());
  assert.ok(!Tendril('$x=(_number where boolean($x))').match(0).hasMatch());
});

// ==================== String Operations ====================

test('guard: string concatenation', () => {
  assert.ok(Tendril('$x=(_string where $x + "!" == "hello!")').match('hello').hasMatch());
});

test('guard: string comparison', () => {
  // Lexicographic comparison
  assert.ok(Tendril('$x=(_string where $x > "abc")').match('abd').hasMatch());
  assert.ok(!Tendril('$x=(_string where $x > "abc")').match('abb').hasMatch());
});
...
test('guard: multi-variable comparison', () => {
  ...
  const pattern = '[$x=(_number where $x < $y) $y=(_number)]';
...
test('guard: filter numbers in array', () => {
  const pattern = '[... $x=(_number where $x > 0) ...]';
...
test('guard: extract even numbers', () => {
  const pattern = '[... $x=(_number where $x % 2 == 0) ...]';
...
test('guard: filter object values', () => {
  ...
  const pattern = '{_:$v=(_number where $v > 20)}';
...
test('guard: key-value relationship', () => {
  ...
  const pattern = '{$k=(_ where size($k) == $v):$v=(_number)}';
...
test('guard: unbound variable at end of match fails', () => {
  ...
  const pattern = '$x=(_number where $x < $y)';
  assert.ok(!Tendril(pattern).match(5).hasMatch());
});

test('guard: literal values in expression', () => {
  assert.ok(Tendril('$x=(_number where $x > 10 && $x < 20)').match(15).hasMatch());
  assert.ok(!Tendril('$x=(_number where $x > 10 && $x < 20)').match(25).hasMatch());
});

test('guard: null comparison', () => {
  assert.ok(Tendril('$x=(_ where $x == null)').match(null).hasMatch());
  assert.ok(!Tendril('$x=(_ where $x == null)').match(0).hasMatch());
});

test('guard: boolean literal', () => {
  assert.ok(Tendril('$x=(_boolean where $x == true)').match(true).hasMatch());
  assert.ok(!Tendril('$x=(_boolean where $x == true)').match(false).hasMatch());
});

test('guard: string literal in expression', () => {
  assert.ok(Tendril('$x=(_string where $x == "hello")').match('hello').hasMatch());
  assert.ok(!Tendril('$x=(_string where $x == "hello")').match('world').hasMatch());
});

test('guard: parentheses for precedence', () => {
  // Without parens: $x > 0 && $x < 10 || $x > 90
  // With parens: $x > 0 && ($x < 10 || $x > 90)
  const p1 = '$x=(_number where $x > 0 && ($x < 10 || $x > 90))';
...
test('guard: unary minus', () => {
  assert.ok(Tendril('$x=(_number where $x == -5)').match(-5).hasMatch());
  assert.ok(Tendril('$x=(_number where -$x > 0)').match(-5).hasMatch());
});
...
test('guard: typed wildcard with guard', () => {
  ...
  const result = Tendril('$x=(_number where $x > 0)').match(42).solutions().first();
  assert.ok(result);
...
test('guard: regex pattern with guard', () => {
  ...
  const pattern = '$x=(/[a-z]+/ where size($x) >= 3)';
...
test('guard: combined with object pattern', () => {
  const pattern = '{age: $x=(_number where $x >= 18)}';
...
test('guard: precedence - multiplication before addition', () => {
  assert.ok(Tendril('$x=(_number where $x * 2 + 3 == 13)').match(5).hasMatch());
  // 5 * 2 + 3 = 13, not 5 * (2 + 3) = 25
});

test('guard: precedence - comparison before logical', () => {
  // $x > 5 && $x < 10 should be ($x > 5) && ($x < 10)
  assert.ok(Tendril('$x=(_number where $x > 5 && $x < 10)').match(7).hasMatch());
});

test('guard: precedence - && before ||', () => {
  // true || false && false should be true || (false && false) = true
  assert.ok(Tendril('$x=(_number where true || false && false)').match(1).hasMatch());
});
